...
""" A script to populate a window with the user's TV Shows and allow them to
select which ones to watch or which ones should be watched in random order """
        ...
        if controlID == SAVE:
            for itm in range(self.item_count):
                if (
                    itm != 0
                    and itm != 1
                    and self.name_list.getListItem(itm).isSelected()
                ):
...
def selection_func():
    ...
    if "tvshows" in all_shows:
        ...
        all_variables = [
            (x["label"], int(x["tvshowid"]), x["thumbnail"]) for x in all_s
        ]
    else:
        all_variables = []
