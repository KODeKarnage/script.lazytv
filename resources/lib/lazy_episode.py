...
class LazyEpisode(object):
    ...
    This class handles retrieving the episode data from the database.
    Its dictionary is merged into the LazyListItem, which then handles
    specific ListItem variable population.
    """
    ...
    def retrieve_details(self, resume_only=False):
        ...
        if ep_details.get("resume", {}).get("position", 0) and ep_details.get(
            "resume", {}
        ).get("total", 0):
            resume = "true"
            played = "%s%%" % int(
