...
class LazyList(threading.Thread):
    ...
    def convert_LzEps_to_LzItms(self, epitems):
        ...
        return [
            ep_pair[0].merge(ep_pair[1]) for ep_pair in LLI if ep_pair[1] is not None
        ]
    ...
    def process_listitems(self):
        ...
        if self.s["excl_randos"]:
            self.listitems = filter(lambda x: x.show_type != "randos", self.listitems)

        if order == 0:

            self.listitems = sorted(
                self.listitems, key=lambda x: x.OrderShowTitle, reverse=reverse
            )

        elif order == 1:

            self.listitems = sorted(
                self.listitems, key=lambda x: x.lastplayed, reverse=reverse
            )
            ...
            self.listitems = sorted(
                self.listitems, key=lambda x: x.stats[3], reverse=reverse
            )

        elif order == 3:

            self.listitems = sorted(
                self.listitems, key=lambda x: x.stats[0], reverse=reverse
            )

        elif order == 4:

            self.listitems = sorted(
                self.listitems, key=lambda x: x.Season, reverse=reverse
            )
...
class LazyListGui(xbmcgui.WindowXMLDialog):
    ...
    def load_data(self, new_listitems):
        ...
        for i, listitem in enumerate(new_listitems):
            ...
            if i == 1000 or (
                self.s.get("limitshows", False) == True
                and i == self.s.get("window_length", -1)
            ):
                break
            ...
            if self.s["skinorno"] == 0:
                ...
                listitem.label2 = (
                    ...
                    if self.s["skinorno"] != 0
                    else str(listitem.PercentPlayed) + str(listitem.lastplayed)
                )
        ...
        for itm in range(self.name_list.size()):
            ...
            if itm == self.pos:
                ...
                if EpID:
                    if self.s["skinorno"] != 0:
                        if (
                            self.name_list.getListItem(itm).getProperty("watched")
                            == "false"
                        ):
                            self.log("toggling from unwatched to watched")
                            self.name_list.getListItem(itm).setProperty(
                                "watched", "true"
                            )
    ...
    def export_selection(self):
        ...
        for itm in range(self.name_list.size()):
            ...
            if self.name_list.getListItem(itm).isSelected() or itm == self.pos:
                ...
                if self.export_list:
                    self.export_list = "".join(
                        [self.export_list, ":-exporter-:", filename]
                    )
                else:
