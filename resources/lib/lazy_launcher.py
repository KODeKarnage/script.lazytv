    ...
    def version_check(self):
        ...
        while True:
            try:
                ...
                # notify the user that the service is unavailable, and ask if
                # they want to restart it
                ans = self.DIALOG.yesno('LazyTV',self.lang(32106),self.lang(32107))

                if ans == 1:
                    # this will always happen after the first install. The addon
                    # service is not auto started after install.
        ...
        if service_version == 'failed':
            ...
            # this section is to determine if the clone needs to be up-dated
            # with the new version it checks the clone's version against the services version.
            if clone_upd == 1:
