...
class LazyInteraction(object):
	...
	def internal_notification(self, next_ep):
		''' Displays the next up notification if the second trigger has been pulled and something is still playing. '''

		self.log('next_prompt dialog method reached, showtitle: {}, season: {}, episode: {}'.format(next_ep.show_title, next_ep.Season, next_ep.Episode))
	...
	def external_notification(self, next_ep):
		''' Displays the next up notification if the first trigger has been pulled the episode has stopped or has ended. '''
		...
		if self.release == 'Frodo':
			if self.s['promptduration']:
				prompt = self.DIALOG.select(self.lang(32164), [self.lang(32165) % self.s['promptduration'], self.lang(32166) % (next_ep.show_title, SE)], autoclose=int(self.s['promptduration'] * 1000))
			else:
				prompt = self.DIALOG.select(self.lang(32164), [self.lang(32165) % self.s['promptduration'], self.lang(32166) % (next_ep.show_title, SE)])

		else:
			if self.s['promptduration']:
				prompt = self.DIALOG.yesno(self.lang(32167) % self.s['promptduration'], self.lang(32168) % (next_ep.show_title, SE), self.lang(32169), yeslabel = ylabel, nolabel = nlabel, autoclose=int(self.s['promptduration'] * 1000))
			else:
				prompt = self.DIALOG.yesno(self.lang(32167) % self.s['promptduration'], self.lang(32168) % (next_ep.show_title, SE), self.lang(32169), yeslabel = ylabel, nolabel = nlabel)
