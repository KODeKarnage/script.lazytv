...
class LazyTrackingImp(object):
    """ The IMP will monitor the episode that is playing and put an Alert to
    Main when it passes a certain point in its playback. """
    ...
    def begin_monitoring_episode(self, showid, duration):
        ...
        self.log(
            "IMP monitor starting: showid= {}, duration = {}".format(showid, duration)
        )
    ...
    def playlist_monitoring_daemon(self, wait_time=15):
        ...
        while all(
            [
                ...
                not self.abandon_playlist_daemon,
            ]
        ):
            xbmc.sleep(interval)
    ...
    def monitor_daemon(self):
        """ This loops until either self.active is set to false, xbmc requests
        an abort, or the current position in the episode exceeds the trigger_point.
        It is spawned in its own thread. It sleeps for a second between loops to
        reduce system load.
        """
        ...
        while self.episode_active_trigger and not self.abort_tracking:
            ...
            current_position = self.runtime_converter(
                xbmc.getInfoLabel("VideoPlayer.Time")
            )
        ...
        while (
            self.episode_active_nextup
            and not xbmc.abortRequested
            and not self.abort_tracking
        ):
