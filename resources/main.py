...
def next_show_engine(showid, epid=[], eps=[], Season="null", Episode="null"):
    ...
    ep_details_query = {
        ...
        "params": {"properties": ["season", "episode"], "episodeid": next_ep},
        "id": "1",
    }
    ...
    if "episodedetails" in epd and epd["episodedetails"]:
        return (
            next_ep,
            [epd["episodedetails"]["season"], epd["episodedetails"]["episode"], newod, next_ep],
        )
...
def sort_shows(nepl_retrieved, nepl_stored):
    ...
    if sort_by == 0:
        ...
        log("sort by name")
        nepl_inter = [
            [x["label"], day_conv(x["lastplayed"]) if x["lastplayed"] else 0, x["tvshowid"]]
            for x in nepl_retrieved
            if x["tvshowid"] in nepl_stored
        ]
        ...
        nepl_inter = [
            [
                int(WINDOW.getProperty("%s.%s.CountonDeckEps" % ("LazyTV", x["tvshowid"]))),
        ...
        nepl_inter = [
            [
                int(WINDOW.getProperty("%s.%s.CountWatchedEps" % ("LazyTV", x["tvshowid"]))),
                day_conv(x["lastplayed"]) if x["lastplayed"] else 0,
                x["tvshowid"],
            ]
            for x in nepl_retrieved
        ...
        nepl_inter = [
            [
                int(WINDOW.getProperty("%s.%s.Season" % ("LazyTV", x["tvshowid"]))),
                day_conv(x["lastplayed"]) if x["lastplayed"] else 0,
                x["tvshowid"],
            ]
            for x in nepl_retrieved
        ...
        nepl_inter = [
            [day_conv(x["lastplayed"]) if x["lastplayed"] else 0, x["tvshowid"]]
            for x in nepl_retrieved
            if x["tvshowid"] in nepl_stored
        ]
...
def random_playlist(population):
    ...
    while (
        count < length and candidate_list
    ):  # while the list isnt filled, and all shows arent abandoned or movies added
        ...
        if candi_type == "t":
            ...
            if curr_candi in list(added_ep_dict.keys()):
                ...
                if (
                    not multipleshows
                ):  # check added_ep list if multiples allowed, if not then abandon the show
                    tg = "t" + str(curr_candi)
                    if tg in candidate_list:
                        candidate_list.remove("t" + str(curr_candi))
            ...
            if not premieres:
                if (
                    WINDOW.getProperty("%s.%s.EpisodeNo" % ("LazyTV", curr_candi)) == "s01e01"
                ):  # if desired, ignore s01e01
                    tg = "t" + str(curr_candi)
                    if tg in candidate_list:
                        candidate_list.remove("t" + str(curr_candi))
            ...
            if not multi:
                if multipleshows:
                    ...
                    added_ep_dict[curr_candi] = [
                        WINDOW.getProperty("%s.%s.Season" % ("LazyTV", curr_candi)),
                        WINDOW.getProperty("%s.%s.Episode" % ("LazyTV", curr_candi)),
                        eps_list,
                        WINDOW.getProperty("%s.%s.EpisodeID" % ("LazyTV", curr_candi)),
                    ]
                else:
                    added_ep_dict[curr_candi] = ""
            else:
    ...
    WINDOW.setProperty(
        "%s.playlist_running" % ("LazyTV"), "true"
    )  # notifies the service that a playlist is running
    WINDOW.setProperty(
        "LazyTV.rando_shuffle", "true"
...
def create_next_episode_list(population):
    ...
    while stay_puft and not monitor.abortRequested():
        ...
        if open_addon_window:
            log(
                "Opening addon window, existing window %s"
        ...
        if self.load_items:
            ...
            for i, show in enumerate(self.data):
                ...
                if skin != 0:
                    ...
                    self.fanart = WINDOW.getProperty(
                        "%s.%s.Art(tvshow.fanart)" % ("LazyTV", show[1])
                    )
                    self.numwatched = WINDOW.getProperty(
                        "%s.%s.CountWatchedEps" % ("LazyTV", show[1])
                    )
                    self.numondeck = WINDOW.getProperty(
                        "%s.%s.CountonDeckEps" % ("LazyTV", show[1])
                    )
                    try:
                        self.numskipped = str(
                    ...
                    self.title = "".join(
                        [
                            WINDOW.getProperty("%s.%s.TVshowTitle" % ("LazyTV", show[1])),
                            " ",
                            WINDOW.getProperty("%s.%s.EpisodeNo" % ("LazyTV", show[1])),
                        ]
                    )
    ...
    def data_refresh(self):
        ...
        for item_position in range(item_count):
            ...
            if skin != 0:
                ...
                title = "".join(
                    [
                        WINDOW.getProperty("%s.%s.TVshowTitle" % ("LazyTV", show_id)),
                        " ",
            ...
            list_item_show.setInfo(
                "video", {"season": season, "episode": episode, "plot": plot, "title": eptitle}
            )
...
def run():
    ...
    if not service_lives:
        ...
        if ans == 1:
            ...
            xbmc.executeJSONRPC(
                '{"jsonrpc":"2.0","method":"Addons.SetAddonEnabled","id":1,"params":{"addonid":"script.lazytv","enabled":false}}'
            )
            xbmc.sleep(1000)
            xbmc.executeJSONRPC(
                '{"jsonrpc":"2.0","method":"Addons.SetAddonEnabled","id":1,"params":{"addonid":"script.lazytv","enabled":true}}'
            )
        ...
        if (
            version_string_to_tuple(str(__addonversion__))
            != version_string_to_tuple(str(service_version))
        ...
        if (
            ...
            if clone_upd == 1:
                ...
                xbmc.executebuiltin(
                    "RunScript(%s,%s,%s,%s,%s)"
                    % (update_script, service_path, scriptPath, __addonid__, scriptName)
                )
                sys.exit()
